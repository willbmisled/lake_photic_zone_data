---
title: "predicted_summer_lake_temperatures"
author: "bryan"
date: "August 1, 2019"
...
# locations of data
db_tmeans <- "L:/Public/Milstead_Lakes/prism/lake_temperatures.sqlite"
db_preds <- "L:/Public/Milstead_Lakes/prism/lake_temperatures_predicted.sqlite"
# db_preds <- "c:/bryan/lake_temperatures_predicted.sqlite"

rf_location <- "L:/Public/Milstead_Lakes/prism/RFAll20190503.Rds"
```
...
* The model was based on measured photic zone temperatures from the 2007 and 2012 National Lake assessment and a series of predictor variables.  Response and predictor variables were combined in a single database ("L:/Public/Milstead_Lakes/prism/lake_temperatures.sqlite"). The database is documented in "lake_temperatures_data.Rmd".
...
* The predicted summer temperatures were stored in "L:/Public/Milstead_Lakes/prism/lake_temperatures_predicted.sqlite"
* This document shows the steps used to create the predictions.  It takes about a week to run on a dedicated computer.

## Data 
* "RFAll20190503.Rds" ("L:/Public/Milstead_Lakes/prism/RFAll20190503.Rds") is the random forest model object used for the predictions
* All predictor data needed are in: "L:/Public/Milstead_Lakes/prism/lake_temperatures.sqlite"
* Data will be joined from two tables:

### Table "lakes" in lake_temperatures.sqlite; observations = 363,300
...
### Table "tmeans"in lake_temperatures.sqlite; observations = 651,830,628
...
* data predictions are here: "L:/Public/Milstead_Lakes/prism/lake_temperatures_predicted.sqlite" 
* This database has two tables:
    - **p_day**: 
    - **p_means**:

### Table "p_day" in lake_temperatures_predicted.sqlite; observations = 1,639,936,200

field	| source | units | indexed |	primary |	description
----------- | -----------	| ------- | ---- | ---- |	--------------------------------- 
**comid**|lmorpho|int|YES|composite|nhdplus comid from the lmorpho dataset
**date**|user defined |YYYYMMDD|YES|composite|date of the lake photic zone temperature estimate
**predicted**|predicted|degrees C|NO|NO|predicted lake photic zone temperature

### Table "p_means" in lake_temperatures_predicted.sqlite; observations = 80,652,600

field	| source | units | indexed |	primary |	description
----------- | -----------	| ------- | ---- | ---- |	--------------------------------- 
**comid**|lmorpho|int|YES|composite|nhdplus comid from the lmorpho dataset
**year**|user defined |YYYY|NO|composite|year for the lake photic zone temperature prediction; values = c(1981 to 2017)
**mo**|user defined |MM|NO|composite|month for the lake photic zone temperature prediction; values = c(NA, 6, 7, 8, 9)
**season**|user defined |char|NO|composite|season for the lake photic zone temperature prediction; c("NA", "june2aug", "sostice2equinox")
**mean**|calculated|degrees C|NO|NA|mean temperature for month or season
**sd**|calculated|degrees C|NO|NA|std deviation of temperature for month or season
**z**|calculated|NA|NO|NA|z score of temperature for month or season
...
* write function to predict daily lake photic zone temperatures and monthly & seasonal means & z scopes for lakes from table "lakes" in "lake_temperatures.sqlite"
    * join to tmeans data (table "tmeans" in "lake_temperatures.sqlite") for dates
...
* get data for all lakes in table "lakes" in "lake_temperatures.sqlite"
